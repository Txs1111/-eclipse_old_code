...
<%	
		...
		if(sub==null||"".equals(sub)||"null".equals(sub)){//初始化进入界面时的数据
			...
			if("".equals(pagedata)||"null".equals(pagedata)||pagedata==null){
				pagedata=bean.first_page.replace(">", ">\n");
			}
		}
		if("1".equals(sub)){
	...
	%>
		...
		<div style="width:50%;height:550px;float:left;flow-over:hidden;">
			...
			<div style="width:100%;height:500px;float:left;border:1px solid gray;">			<%
...
*个人主页被分为两种模块：大模块宽度为页面的百分之百，小模块为百分之五十，高度都为500px;
  ...
  保存/读取：保存也不会把代码展示到个人主页，只会将数据上传至云端，可在下一次编辑的时候点击”读取“加载上一次编辑的内容
